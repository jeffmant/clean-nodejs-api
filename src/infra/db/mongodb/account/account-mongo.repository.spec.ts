...
describe('Account MongoDB Repository', () => {
  ...
  describe('getByToken', () => {
    ...
    test('Should return an account on getByToken with admin role', async () => {
      const sut = makeSut()
      await accountCollection.insertOne({
        name: 'valid_name',
        email: 'valid_email@email.com',
      ...
      const account = await sut.getByToken('any_token', 'admin')
      expect(account).toBeTruthy()
      expect(account.id).toBeTruthy()
      expect(account.name).toBe('valid_name')
      expect(account.email).toBe('valid_email@email.com')
    ...
    test('Should return an account on getByToken with invalid role', async () => {
      const sut = makeSut()
      await accountCollection.insertOne({
        name: 'valid_name',
        email: 'valid_email@email.com',
    ...
    test('Should return an account on getByToken if user is admin role', async () => {
      const sut = makeSut()
      await accountCollection.insertOne({
        name: 'valid_name',
        email: 'valid_email@email.com',
